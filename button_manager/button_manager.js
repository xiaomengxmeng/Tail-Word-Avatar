...
window.editButton = function(index) {
    ...
    // 小尾巴功能开关设置
    let showSuffixButton = localStorage.getItem('showSuffixSettingsButton') !== 'false';
    ...
    function getCurrentSuffixIndex() {
        const index = parseInt(localStorage['xwb_suffix_index']);
        return isNaN(index) || index < 0 ? 0 : index;
    }

    function getCurrentSuffixText() {
        // 小尾巴选项数组（与tail_word_avatar.js保持一致）
        const suffixOptions = [
            '时光清浅处，一步一安然。',
            '心若向阳，无畏悲伤。',
            '岁月静好，现世安稳。',
        ...
        // 优先检查是否有自定义小尾巴
        const isCustom = localStorage['xwb_is_custom_suffix'] === 'true';
        const customSuffix = localStorage['xwb_custom_suffix'];
    ...
    // 在按钮管理面板中添加小尾巴设置功能
    function addSuffixSettingsToPanel(panel, contentContainer) {
        // 小尾巴设置区域
        const suffixSettingsSection = document.createElement('div');
        suffixSettingsSection.style.cssText = `
            background: #f0f8ff;
        ...
        suffixTitle.style.color = '#0066cc';
        suffixTitle.style.fontSize = '16px';
        suffixSettingsSection.appendChild(suffixTitle);
        ...
        suffixSettingsSection.appendChild(previewDiv);
        ...
        toggleSwitch.checked = localStorage['xwb_flag'] !== 'false';
        ...
        toggleSwitch.onchange = () => {
            localStorage['xwb_flag'] = toggleSwitch.checked;
        };
        toggleDiv.appendChild(toggleLabel);
        toggleDiv.appendChild(toggleSwitch);
        suffixSettingsSection.appendChild(toggleDiv);
        ...
        wordCountInput.value = parseInt(localStorage['xwb_tail_word_count']) || 5;
        ...
        wordCountDisplay.textContent = parseInt(localStorage['xwb_tail_word_count']) || 5;
        ...
        customSuffixTextarea.style.cssText = `
            ...
            box-sizing: border-box;
        `;
        customSuffixTextarea.value = localStorage['xwb_custom_suffix'] || '';
        customSuffixTextarea.oninput = () => {
            const customText = customSuffixTextarea.value.trim();
            if (customText) {
                localStorage['xwb_custom_suffix'] = customText;
                localStorage['xwb_is_custom_suffix'] = 'true';
                // 更新预览
        ...
        suffixSettingsSection.appendChild(customSuffixDiv);
        ...
        suffixSelect.onchange = () => {
            ...
            if (selectedValue.startsWith('preset-')) {
                ...
                localStorage['xwb_suffix_index'] = index;
                if (localStorage['xwb_is_custom_suffix']) {
                    delete localStorage['xwb_is_custom_suffix'];
                }
                // 更新预览
                previewContent.textContent = suffixOptions[index];
        ...
        suffixSettingsSection.appendChild(suffixSelectDiv);

        // 是否显示小尾巴设置按钮的开关
        const showButtonDiv = document.createElement('div');
        showButtonDiv.style.cssText = `
            display: flex;
            align-items: center;
            justify-content: space-between;
            margin-bottom: 15px;
        ...
        showButtonLabel.textContent = '显示小尾巴设置按钮：';
        showButtonLabel.style.fontWeight = 'bold';
        const showButtonCheckbox = document.createElement('input');
        showButtonCheckbox.type = 'checkbox';
        showButtonCheckbox.checked = showSuffixButton;
        showButtonCheckbox.style.cssText = `
            width: 20px;
            height: 20px;
            cursor: pointer;
        `;
        showButtonCheckbox.onchange = () => {
            showSuffixButton = showButtonCheckbox.checked;
            localStorage.setItem('showSuffixSettingsButton', showSuffixButton);
            showNotification('设置已保存，刷新页面后生效');
        };
        showButtonDiv.appendChild(showButtonLabel);
        showButtonDiv.appendChild(showButtonCheckbox);
        suffixSettingsSection.appendChild(showButtonDiv);

        // 保存按钮
        const saveBtn = document.createElement('button');
        saveBtn.textContent = '保存小尾巴设置';
        saveBtn.style.cssText = `
            background: linear-gradient(135deg, #52c41a 0%, #73d13d 100%);
            color: white;
            border: none;
        ...
        saveBtn.addEventListener('mouseenter', () => {
            ...
            saveBtn.style.boxShadow = '0 4px 12px rgba(82, 196, 26, 0.4)';
        });
        saveBtn.addEventListener('mouseleave', () => {
            saveBtn.style.transform = 'translateY(0)';
            saveBtn.style.boxShadow = '0 2px 8px rgba(82, 196, 26, 0.3)';
        });
        saveBtn.onclick = () => {
            // 保存所有设置
            localStorage['xwb_flag'] = toggleSwitch.checked;
            ...
            showNotification('小尾巴设置已保存！');
        };
        suffixSettingsSection.appendChild(saveBtn);

        contentContainer.appendChild(suffixSettingsSection);
    }

    // 重写openButtonManagerPanel函数，添加小尾巴设置
    const originalOpenButtonManagerPanel = openButtonManagerPanel;
    openButtonManagerPanel = function() {
        originalOpenButtonManagerPanel();
        
        // 在面板中添加小尾巴设置区域
        const panel = document.getElementById('button-manager-panel');
        if (panel) {
            const contentContainer = panel.querySelector('div[style*="padding: 20px"]');
            if (contentContainer) {
                addSuffixSettingsToPanel(panel, contentContainer);
            }
        }
    };
})();
